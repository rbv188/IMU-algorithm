...
{
    euler_angles result;
    double q0 = q.a;//double(q.a);
    double q1 = q.b;//double(q.b);
    double q2 = q.c;//double(q.c);
    double q3 = q.d;//double(q.d);
    ...
    /*result.roll = atan2(2*(q2*q0 - q1*q3),1 - 2*(q2*q2 + q3*q3))*180/3.14;
    result.pitch = asin(2*(q1*q2 + q3*q0))*180/3.14;
    result.yaw = atan2(2*(q1*q0 - q2*q3),1 - 2*(q1*q1 + q3*q3))*180/3.14;*/
    return result;
}
